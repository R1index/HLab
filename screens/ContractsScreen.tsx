...
import { 
    ...
    TrendingUp, Timer, AlertOctagon, DollarSign, Gem
} from 'lucide-react';
...
interface Props {
  state: GameState;
  onCompleteContract: (contractId: string, success: boolean, score: number) => void;
            ...
            {factions.map(faction => {
                ...
                return (
                    <button
                        ...
                        className={`
                            relative flex flex-col items-center p-3 rounded-xl border transition-all duration-300 min-w-[100px] group
                            ${isSelected 
                                ? `${styles.bgTrans} ${styles.border} shadow-[0_0_15px_rgba(0,0,0,0.3)] scale-105 z-10` 
                                : 'bg-slate-900 border-slate-800 opacity-70 hover:opacity-100 hover:bg-slate-800'
                            }
                        `}
                    >
                        <div className={`
                            w-8 h-8 rounded-full flex items-center justify-center mb-2 transition-all
                            ${isSelected ? `${styles.dot} shadow-[0_0_10px_currentColor]` : 'bg-slate-800'}
                        `}>
                            <Hexagon size={16} className={isSelected ? 'text-white' : styles.text} />
                        </div>
                        
                        <div className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${isSelected ? 'text-white' : 'text-slate-500'}`}>
                            {faction.name}
                        </div>

                        {/* Level Indicator */}
                        <div className={`text-[9px] font-mono ${isSelected ? 'text-cyan-400' : 'text-slate-600'}`}>
                            LVL {faction.level}
                        </div>
                    </button>
                );
...
const FactionHeader = ({ faction, directorId }: { faction: Faction, directorId: string }) => {
    ...
    return (
        <div className={`
            relative w-full bg-slate-900/80 border ${styles.borderDim} rounded-xl overflow-hidden mb-6 p-4 md:p-6
        ...
        `}>
            ...
            <div className="flex flex-col md:flex-row gap-6 relative z-10">
                ...
                <div className="flex items-start gap-4">
                    <div className={`w-16 h-16 rounded-lg border-2 ${styles.border} overflow-hidden shadow-lg shrink-0 bg-slate-950 group`}>
                        {director ? (
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <h2 className={`text-2xl font-bold text-white leading-none tracking-tight`}>{faction.name}</h2>
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${styles.borderDim} ${styles.text} bg-slate-950/50 uppercase`}>
                                LEVEL {faction.level}
                            </span>
                        </div>
                        <p className={`text-xs ${styles.textLight} font-mono italic opacity-80 mb-3`}>"{quote}"</p>
                        
                        <div className="space-y-1">
                            <div className="flex justify-between text-[10px] font-mono text-slate-400">
                                <span className="flex items-center gap-1"><TrendingUp size={10} /> REPUTATION PROGRESS</span>
                            ...
                            <div className="h-1.5 w-full md:w-64 bg-slate-950 rounded-full overflow-hidden border border-slate-800/50">
                ...
                {/* Active Bonuses Grid */}
                <div className="flex-1 md:border-l border-slate-800/50 md:pl-6 grid grid-cols-2 md:grid-cols-3 gap-2 content-center">
                     {Object.entries(bonuses).map(([r, bonus]) => {
                         ...
                         return (
                             <div key={r} className={`
                                ...
                                ${isUnlocked 
                                    ? 'bg-slate-950/60 border-slate-700 opacity-100' 
                                    : isNext 
                                        ? 'bg-slate-900/30 border-slate-800/50 opacity-60' 
                                        : 'bg-transparent border-transparent opacity-30'}
                             `}>
                                 <div className={`w-1.5 h-1.5 rounded-full ${isUnlocked ? 'bg-green-400 shadow-[0_0_5px_currentColor]' : 'bg-slate-600'}`} />
                                 <div className="flex flex-col">
                                     <div className="flex items-center gap-1">
                                        <span className="text-[9px] uppercase font-bold text-slate-500">{r}</span>
                                        {isNext && <span className="text-[8px] text-cyan-500 bg-cyan-950/30 px-1 rounded border border-cyan-900">NEXT</span>}
                                     </div>
                                     <span className={`text-xs font-mono ${isUnlocked ? 'text-white' : 'text-slate-600'}`}>{String(bonus)}</span>
                                 </div>
                             </div>
                         )
                     })}
...
const ContractCard = React.memo(({ 
    ...
    if (isTrade) {
        ...
        shadowClass = 'shadow-[0_0_15px_rgba(220,38,38,0.1)]';
    } else if (isExtreme) {
    ...
    return (
        <div className={`
            ...
            ${canAfford || isTrade ? 'hover:scale-[1.02] hover:bg-slate-900 hover:border-opacity-100' : ''}
        `}>
            {/* Header */}
            <div className="p-3 flex justify-between items-start border-b border-slate-800/50 bg-slate-950/30">
                <div className="flex flex-col gap-1.5 min-w-0">
                    <div className="flex items-center gap-2">
                        ...
                        {!isTrade && <span className="text-[10px] text-slate-500 font-mono">TIER {contract.tier}</span>}
                    </div>
                    ...
                    <div className="space-y-3">
                          <div className="bg-emerald-900/10 border border-emerald-500/20 p-2 rounded">
                            ...
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-mono text-emerald-300">{contract.tradeReqStat} &gt; {contract.tradeReqValue}</span>
                            </div>
                        </div>
                    </div>
                ) : (
            ...
            <button
                ...
                className={`
                    ...
                    ${(isTrade || canAfford) 
                        ? `${isTrade ? 'bg-emerald-700 hover:bg-emerald-600' : styles.btn} ${styles.btnHover} text-white` 
...
export const ContractsScreen: React.FC<Props> = ({ state, onCompleteContract, onFulfillTrade, bonuses, updateResources }) => {
  ...
  const handleStart = useCallback((contract: Contract) => {
    ...
    setIsAccepting(true);
    updateResources({ credits: -cost });
    playSound('click'); 
    spawnText(window.innerWidth / 2, window.innerHeight / 2, `-${cost} CR`, "text-red-400", "md");
  ...
  }, [isAccepting, state.resources.credits, updateResources, spawnText]);
  ...
  const handleGameComplete = useCallback((success: boolean, score: number) => {
      ...
      if (activeContract) {
            ...
            if (success) {
                const creditBonus = activeContract.rewardCredits * (bonuses.creditMult - 1);
                const dataBonus = activeContract.rewardData * (bonuses.dataMult - 1);
                const baseGems = activeContract.rewardGems || 0;
                const gemBonus = baseGems * (bonuses.gemMult - 1);
                
                creditReward = Math.floor(activeContract.rewardCredits + creditBonus);
                dataReward = Math.floor(activeContract.rewardData + dataBonus);
                gemsReward = Math.floor(baseGems + gemBonus);
                ...
                setTimeout(() => spawnText(cx, cy - 50, `+${creditReward} CREDITS`, "text-yellow-400", "lg"), 100);
                setTimeout(() => spawnText(cx, cy, `+${dataReward} DATA`, "text-cyan-400", "lg"), 300);
                if (gemsReward > 0) {
                    setTimeout(() => spawnText(cx, cy + 50, `+${gemsReward} GEMS`, "text-purple-400", "lg"), 500);
                }
            }
            onCompleteContract(activeContract.id, success, score);
      }
      setActiveContract(null);
  }, [activeContract, bonuses, updateResources, onCompleteContract, spawnText]);
  ...
  // Render MiniGame View
  if (activeContract) {
    return (
        <div className="h-full flex flex-col animate-in fade-in duration-300">
            <div className={`mb-4 flex items-center justify-between p-3 rounded-lg border backdrop-blur-sm ${activeContract.difficulty === 'Black Ops' ? 'bg-red-950/80 border-red-800' : 'bg-slate-900/80 border-slate-700'}`}>
                <div className="flex flex-col">
                    <span className="text-[10px] text-slate-400 font-mono">ACTIVE PROTOCOL</span>
            ...
            <div className="flex-1 border border-slate-700 rounded-xl bg-slate-950 relative overflow-hidden shadow-2xl">
                <MiniGame 
    ...
    <div className="flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex items-center justify-between mb-4 px-2">
          <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-300 tracking-widest">CONTRACT NETWORK: ONLINE</span>
          </div>
          <div className="text-[10px] text-slate-500 font-mono">
              SECURE CONNECTION // {state.factions[selectedFactionId].name.toUpperCase()}
          </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-y-auto pr-1 pb-20">
          ...
          <div>
                <div className="flex items-center gap-2 mb-3 px-2">
                    ...
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Available Operations ({factionContracts.length})</span>
                </div>
